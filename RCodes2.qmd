...
2.  What does the `bill_depth_mm` variable in the `penguins` data frame describe?
    ...
    a number denoting bill length (milimeters)
...
3.  Make a scatterplot of `bill_depth_mm` vs. `bill_length_mm`.
    ...
    The relationship between these two variables is that they're non-linear.
    ```
...
4.  What happens if you make a scatterplot of `species` vs. `bill_depth_mm`?
    ...
    ```{r}
    ggplot(data=penguins,mapping=aes(x=bill_depth_mm, y=species, colour=species))+geom_point()
